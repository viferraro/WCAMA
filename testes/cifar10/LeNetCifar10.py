...
for i in range(1):
    ...
    class LeNet5(nn.Module):
        def __init__(self):
            ...
            self.fc1 = nn.Linear(16 * 5 * 5, 120)  # Corrigido: Ajustar as dimensões para a camada totalmente conectada
        ...
        def forward(self, x):
            ...
            x = x.view(-1, 16 * 5 * 5)  # Ajuste a forma do tensor aqui
            x = self.dropout1(x)
            x = F.relu(self.fc1(x))
            x = self.dropout2(x)
            x = F.relu(self.fc2(x))  # Adicione a função de ativação ReLU aqui
            x = self.fc3(x)
